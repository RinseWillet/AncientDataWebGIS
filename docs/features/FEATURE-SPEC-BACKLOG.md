...
| Story ID | Epic | Story | Status | Priority | Size | Dependencies |
|---|---|---|---|---|---|---|
| E15-1 | E15 | Add an `ImageResizeService` (Java `ImageIO`, no new runtime dependency) that downscales/recompresses a JPEG/PNG/WebP to fit a target byte ceiling, preserving aspect ratio | ✅ Done | High | M | None |
| E15-2 | E15 | Wire `MediaService.upload()` to call `ImageResizeService` instead of rejecting when a file exceeds 10 MB; keep a hard-reject ceiling (50 MB, matching the existing Spring `multipart.max-file-size`/nginx `client_max_body_size` bounds) for files too large or corrupt to process | ✅ Done | High | M | E15-1 |
| E15-3 | E15 | Best-effort: set the output JPEG's JFIF pixel-density metadata to 300 DPI via `IIOMetadata` when resizing a JPEG; skip silently (no error) for formats/cases where this isn't practical | ✅ Done | Medium | S | E15-1 |
| E15-4 | E15 | Surface a non-error notice in `MediaUploadForm` when the uploaded photo was resized (e.g. `MediaAssetDTO` gains a `resized: boolean` flag the upload response returns), so an admin doesn't wonder why their file looks smaller on disk | ✅ Done | Medium | S | E15-2 |
| E15-5 | E15 | Tests: resize triggers only above the threshold, output stays under the ceiling, aspect ratio preserved, DPI tag present on resized JPEGs, hard-reject ceiling still rejects with `MEDIA_FILE_TOO_LARGE`-equivalent error, corrupt/unsupported files still rejected | ✅ Done | High | M | E15-2, E15-3 |
| E15-6 | E15 | Write `ADR-015-oversized-media-resize-strategy.md` (library choice — plain `ImageIO` vs. a resize library — target dimensions/quality search strategy, DPI approach, hard-reject ceiling rationale) | ✅ Done | Medium | S | E15-1 |
...
| E3-9 | E3 | Extend E3-7/E3-8's viewport-gating principle to the static `layersConfig.ts`-driven "Aerial Imagery" exclusive group's three Ruhr `lubi_*` WMS layers (real coverage confirmed via each service's `GetCapabilities`: Uedem/Rees in the west/north to Wickede (Ruhr)/Remscheid in the east/south) — not raster-catalog-driven like E3-7/E3-8, so `WmsLayerConfig` gains optional `bounds`/`zoom` fields instead | ✅ Done | Medium | S | E3-7, E3-8 |
...
Story,E3-9,Gate Aerial Imagery (Ruhr lubi) layer selectability by viewport + zoom,,E3,Medium,2,frontend;raster;performance,"Extend E3-7/E3-8's viewport-gating principle to the three Ruhr lubi_* WMS layers in the static layersConfig.ts-driven Aerial Imagery exclusive group, which is not raster-catalog-driven so WmsLayerConfig gains optional bounds/zoom fields instead of reusing PhysicalLayerState.","An Aerial Imagery entry with bounds/zoom set is selectable only when zoom >= its own zoom.min AND bounds intersect the viewport; gated-off entries are hidden, not disabled; entries without bounds (Topographical, NRW Historical Maps, '1952 Köln') are unaffected",E3-7;E3-8
...
**Status:** E3-1 through E3-9 all delivered (August 2026).
...
**What was delivered (E3-9):**
- Trigger: not from the story queue — surfaced when the project owner noticed the three Ruhr `lubi_*` aerial-photo WMS layers (fixed in the same session as a CARTO-basemap-watermark investigation, see `ADR-014-positron-basemap-vector-tiles.md`) only cover the Ruhr metropolitan area — confirmed against each service's own `GetCapabilities` (`LatLonBoundingBox`: `west 6.227895, south 51.167497, east 8.055436, north 51.860887`, matching the project owner's description: Uedem/Rees in the west/north, Wickede (Ruhr)/Remscheid in the east/south) — yet were selectable from anywhere on the map with zero viewport awareness, unlike every raster-catalog-driven layer group by this point (E3-7/E3-8).
- **Scope confirmed with the project owner:** only the three `lubi_1926`/`lubi_1934`/`lubi_1952` entries. The "1952 Köln" aerial entry (a separate, NRW-statewide WMS service) and the "Historical Maps" NRW topo-map entries (Tranchot/Uraufnahme/Neuaufnahme, also broad-coverage) were left ungated — the project owner only asked about "the lubi aerial photography," and a config entry with no `bounds` set is a no-op under this gate by design, so leaving the others alone needed no special-casing.
- Unlike E3-7/E3-8, this group isn't raster-catalog-driven — it's the static, hand-written `layersConfig.ts` array (`WmsLayerConfig`, from `E9-3`) behind the exclusive radio/checkbox "Aerial Imagery" section, which had no bounds/zoom concept at all before this story. Added optional `bounds?: RasterBounds`/`zoom?: RasterZoom` to `WmsLayerConfig` (reusing the same types `PhysicalLayerState` already uses from `types/raster.ts`), populated only on the three Ruhr entries.
- Added `gateExclusiveLayerConfig` (`useMapInteractions.ts`), mirroring `gateHistoricalMapSheet`'s bounds + per-entry `zoom.min` shape, but taking an `isActive: boolean` parameter in place of `PhysicalLayerState.visible` — the exclusive-group model tracks one active name (`activeAerialLayer`), not a `visible` flag per row. A config with no `bounds` always returns not-gated, which is what keeps every other `layersConfig` entry's behavior unchanged with no group-name branching in the gate itself.
- `zoom.min` set to **9** (one step tighter than `PHYSICAL_MIN_ZOOM_FLOOR`'s regional floor of 8) — a reasonable default matching the scale at which the Ruhr sub-region, rather than the whole NL/DE study area, is what's actually in view; not tuned against real usage data, same caveat E3-7's floor carried.
- Wired the same three places E3-7/E3-8 established: `toggleExclusiveLayer` blocks the *on* transition for a gated-off entry (looked up via a new `findLayerConfig` overload that narrows to `WmsLayerConfig` for `'Historical Maps' | 'Aerial Imagery'` groups, avoiding a manual cast); the existing `moveend`/`zoomend` viewport handler gained a third auto-off block for `activeAerialLayer`; a new memoized `gatedAerialLayerNames: string[]` (added to `LayerPanelState`) drives `LayerPanel.tsx`'s row filtering, recomputed off the existing `viewportVersion` counter.
- `LayerPanel.tsx`'s `renderExclusiveSection` (shared by "Topographical" and "Aerial Imagery") now filters out any name in `gatedAerialLayerNames` before rendering, and shows the same `layer-panel__section-empty-hint` fallback pattern E3-7/E3-8 established when a group has entries but none are currently selectable — "hidden, not disabled," matching precedent exactly. Safe no-op for "Topographical" since that array is only ever populated with Aerial Imagery names.

**Impact:**
- The three Ruhr aerial-photo layers are only selectable once the map is actually panned/zoomed to the Ruhr region — previously selectable (and rendering nothing useful) from anywhere on the map.
- An already-active Ruhr layer left "on" while the user pans/zooms away now deactivates itself instead of continuing to request WMS tiles for an area it no longer covers, the same overload-prevention benefit E3-7/E3-8 delivered for the raster-catalog groups.
- No change to "Topographical", "Historical Maps" (NRW), or the "1952 Köln" entry — all remain always-selectable, unaffected by design rather than by exclusion logic.

**Files changed (frontend):**
- `AncientDataWebGIS_FE/src/components/MapComponent/layersConfig.ts` (`WmsLayerConfig.bounds`/`zoom`, populated on the three Ruhr entries)
- `AncientDataWebGIS_FE/src/components/MapComponent/useMapInteractions.ts` (`gateExclusiveLayerConfig`, `findLayerConfig` overload, `gatedAerialLayerNames`, gated `toggleExclusiveLayer`, viewport-change auto-off extended to `activeAerialLayer`), `useMapInteractions.test.ts` (new)
- `AncientDataWebGIS_FE/src/components/LayerPanel/LayerPanel.tsx` (`renderExclusiveSection` filters gated names, empty-state hint), `LayerPanel.test.tsx` (default `gatedAerialLayerNames: []`, new tests)

**Tests:**
- `useMapInteractions.test.ts` (new, 7 pure-function + 6 end-to-end): `gateExclusiveLayerConfig` mirrors `gateHistoricalMapSheet`'s test suite (never gates the active entry even below zoom.min/out of view; no map yet → not gated; a config with no `bounds` never gated; below/at/above the zoom floor; out-of-view/in-view at or above the floor) plus end-to-end tests against `useLayerPanelControl` driving a real (jsdom) `L.Map`, using the actual `layersConfig` Ruhr entries directly rather than synthetic fixtures: blocks activating an out-of-view or below-floor layer and allows it once panned/zoomed into range; an active layer auto-deactivates after panning out of view or zooming below its floor, and re-activates normally back in range; `gatedAerialLayerNames` reflects all three Ruhr entries when out of range and clears when back in range; "Topographical" is unaffected.
- `LayerPanel.test.tsx` (3 new tests): a gated-off Aerial Imagery row is hidden while its ungated siblings still render; the empty-state hint shows when every Aerial Imagery entry is gated off; "Topographical" is unaffected by a populated `gatedAerialLayerNames`.
- Full frontend suite: 178/178 passing. `npm run lint` (eslint, `--max-warnings 0`), `npx tsc --noEmit`, and `npm run build` all clean.
- Backend: no application code changed (frontend-only story); full backend suite unaffected.

**Migration notes:** None — no schema, config, or deployment changes. Purely client-side gating logic layered onto a static frontend config array.

**ADR:** Not needed. Same reasoning as E3-7/E3-8: a client-side UI gating rule reusing an already-accepted pattern (E3-7/E3-8), introducing no new dependency or infrastructure decision.

**Outstanding / manual follow-up (E3-9, project owner):**
- Live smoke test on a real, authenticated Atlas session (same sandbox limitation noted under E3-3/E3-7/E9 — this session had no test credentials for the `/atlas` route, so verification relied on the end-to-end unit tests against a real jsdom `L.Map` instance instead) — to confirm the panel's hide/show feels correct against real pan/zoom interaction and that `zoom.min: 9` is the right feel in practice, not just in the unit tests.
