...
- **GeoServer is already deployed** (`kartoza/geoserver:2.24.1`, service `geoserver` in `AncientDataWebGIS/docker-compose.yml`), data dir bind-mounted at `/volume1/docker/ancientdata/geoserver`, on the same `webgis-edge` Docker network as the `ancientdata` backend. Today it is used **only** for direct QGIS/admin access — LAN IP (`192.168.2.13:2675`) at home, or Cloudflare WARP Private Network routing when off-LAN (`ancientdataworkspace/deploy/README.md` §8). It is not reachable from the public internet.
